...
class TestDeployRewind(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_read_file_with_filedata(self):
        res_1 = json.loads(read_file(FILENAME, FILEPATH))
        if len(res_1) > 0:
            self.assertListEqual(list(res_1[0].keys()), NODE_LIST, "Not matched keys with data received from file")

    def test_read_node_list_json(self):
        node_list = read_node_list_json()
        self.assertEqual(len(node_list), 3, "Didn't get all 3 list of nodes based on os_type")
        self.assertTrue(isinstance(node_list[0][0], dict))
    ...
    @patch("run_app.run_cd_rewind_service")
    @patch("run_app.read_node_list_json")
    ...
    def test_main_happy_path(self, mock_input_parser, mock_logger, mock_read_node_list_json, mock_run_service):
        ...
        start_log_calls = [c for c in mock_logger.info.call_args_list if "CD rewind process started" in c.args[0]]
    ...
    @patch("run_app.run_cd_rewind_service", side_effect=RuntimeError("boom"))
    ...
    def test_main_wraps_exceptions(self, mock_input_parser, mock_logger, mock_read_node_list_json, mock_run_service):
        # Arrange
        mock_input_parser.return_value = self.fake_args
        mock_run_service.side_effect = partial(mock_cd_rewind_exception)
    ...
    @patch("run_app.run_cd_rewind_service", side_effect=ValueError("service error"))
